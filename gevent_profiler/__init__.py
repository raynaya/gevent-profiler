...
_raw_output_file = open('raw_output.txt', 'w')

_print_percentages = True
...
def _print_state(state, depth=0):
    ...
    for call in state.calls:
        if call.elapsed > (state.elapsed * wt_percentage):
            _print_state(call, depth + 2)


def _print_all_state(state, depth=0):
    _maybe_write(_raw_output_file, "%s %s %f\n" %
                 ("." * depth, str(state), state.elapsed))
    for call in state.calls:
        _print_all_state(call, depth + 2)
...
def _print_output(duration):
    ...
    col_names = ["Call Name", "Count", "Cumulative",
                 "Own Cumul", "Child Cumul", "Per Call", "Own/Total"]
    ...
    for _, c in call_list:
        ...
        col_data = [c.name, "%d" % c.count, "%12f" % cumulative, "%12f" %
                    own_cumulative, "%12f" % children_cumulative, "%12f" % per_call, own_ratio]
    ...
    # build row strings
    fmt_out = [" ".join([x.ljust(widths[i])
                         for i, x in enumerate(row)]) for row in output]
    ...
    for gl in _states.keys():
        ...
        _maybe_write(_raw_output_file, "%s\n" % gl)
        _print_state(_states[gl])
        _print_all_state(_states[gl])
        _maybe_write(_summary_output_file, "\n")
        _maybe_write(_raw_output_file, "\n")
    _maybe_flush(_summary_output_file)
    _maybe_flush(_raw_output_file)
...
def attach_on_signal(signum=signal.SIGUSR1, duration=60):
    ...
    """
    def new_handler(signum, frame): return attach(duration=duration)
